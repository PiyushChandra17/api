...
exports.links = async (req, res) => {
  try {
    ...
    const all = await Link.find().skip((page - 1) * perPage).populate("postedBy", '_id name').sort({ createdAt: -1 }).limit(perPage)
    res.json(all)
  } catch (err) {
    console.log(err)
  }
...
exports.viewCount = async (req, res) => {
  try {
    const link = await Link.findByIdAndUpdate(req.params.linkId, { $inc: { views: 1 } }, { new: true })
...
exports.unlike = async (req, res) => {
  try {
    const link = await Link.findByIdAndUpdate(req.body.linkId, { $pull: { likes: req.user._id }}, { new: true }).populate("postedBy", "_id name")
    res.json(link)
  } catch (err) {
    console.log(err)
  }
